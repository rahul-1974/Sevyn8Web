...
var T="#00D4AA", BL="#38BDF8", V="#A78BFA", AM="#FBBF24", PK="#F472B6";
var BG="#060810", B2="#0B0F18", B3="#141924", TX="#E2E8F0", MT="#64748B", DM="#3B4559", BD="rgba(140,160,200,0.07)";
var hd={fontFamily:"'Outfit Variable',Outfit,system-ui",letterSpacing:"-.03em"};
var bn={display:"inline-flex",alignItems:"center",gap:6,padding:"12px 24px",background:T,color:BG,fontWeight:600,fontSize:14,border:"none",borderRadius:6,cursor:"pointer",fontFamily:"inherit"};
...
function Tg(p){return <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:3,color:p.c||T,marginBottom:12,fontWeight:500}}>{p.children}</div>}
...
function HeroAnim(){
  ...
  useEffect(function(){
    ...
    var rgb=function(c2){return c2===T?"0,212,170":c2===BL?"56,189,248":c2===V?"167,139,250":c2===AM?"251,191,36":"244,114,182"};
    ...
    var draw=function(t){
      ...
      x.shadowColor="rgba(0,212,170,"+(0.15*gp)+")";x.shadowBlur=50;
      rr(bx-bw/2,by-bh/2,bw,bh,14);x.fillStyle=B2;x.fill();x.shadowBlur=0;
      x.strokeStyle="rgba(0,212,170,"+(0.3+gp*.15)+")";x.lineWidth=1.2;x.stroke();
      x.setLineDash([3,5]);x.lineWidth=.5;
      for(var i=0;i<4;i++){var ly=H*(.15+i*.23);x.beginPath();x.moveTo(10,ly);var mx=(10+bx-bw/2)/2;x.bezierCurveTo(mx,ly,mx,by+(i-1.5)*20,bx-bw/2,by+(i-1.5)*20);x.strokeStyle="rgba(0,212,170,.08)";x.stroke();if(Math.random()<.03)sp(10,ly,bx-bw/2,by+(i-1.5)*20,T)}
  ...
  var outs=[{l:"Dashboard",c:T},{l:"Smart Alert",c:AM},{l:"OTA Update",c:V},{l:"Partner API",c:BL}];
  var lb={position:"absolute",padding:"8px 14px",borderRadius:7,background:B3,fontSize:13,fontWeight:600,color:"#94A3B8",whiteSpace:"nowrap",border:"1px solid rgba(0,212,170,.18)",zIndex:3};
  return (<div style={{position:"relative",width:"100%",height:"100%"}}>
    ...
    <div style={{position:"absolute",left:"50%",top:"50%",transform:"translate(-50%,-50%)",width:130,height:170,borderRadius:14,border:"1.5px solid "+T+"40",background:B2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:7,zIndex:3,boxShadow:"0 0 50px "+T+"10"}}>
      ...
      {[{l:"HAL",c:T},{l:"AI Engine",c:BL},{l:"Decisioning",c:V}].map(function(la,i){return <div key={i} style={{padding:"4px 12px",borderRadius:5,border:"1px solid "+la.c+"35",background:la.c+"10",fontSize:11,fontWeight:600,color:la.c}}>{la.l}</div>})}
    </div>
...
function BoxSchematic(){
  var layers=[{n:"System-Wide Learning",sub:"The moat — observes all outcomes, compounds fleet-wide, OTA to every device",c:AM},{n:"Integrations",sub:"B2B dashboards · B2C app · OEM white-label · Partner APIs",c:PK},{n:"Contextual Decisioning",sub:"Learns routines · Baselines · 3 AM person ≠ 3 PM person",c:V},{n:"Edge AI Engine",sub:"YOLOv8 · EfficientNet · INT8 quantised · <100ms · <2W",c:BL},{n:"HAL",sub:"Single API across multiple ARM and x86 SoC families. New hardware becomes a supply decision, not an engineering rewrite.",c:T}];
...
function Fleet(){var p2=useState(-1),p=p2[0];useEffect(function(){var id=setInterval(function(){p2[1](function(v){return(v+1)%6})},1000);return function(){clearInterval(id)}},[]);
  ...
  return (<div style={{position:"relative",height:300,maxWidth:560}}><div style={{position:"absolute",left:"50%",top:"50%",transform:"translate(-50%,-50%)",width:90,height:90,borderRadius:"50%",background:"radial-gradient(circle,"+T+"1A,transparent)",border:"1.5px solid "+T+"40",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",zIndex:2,boxShadow:"0 0 40px "+T+"14"}}><div style={{fontSize:13,fontWeight:700,color:T}}>OTA</div><div style={{fontSize:10,color:MT}}>Fleet update</div></div>{N.map(function(n,i){return <div key={i}><svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}}><line x1={n.x+"%"} y1={n.y+"%"} x2="50%" y2="50%" stroke={i<=p?T:BD} strokeWidth={i===p?1.5:.5} strokeDasharray={i<=p?"none":"4 7"} style={{transition:"all .5s"}} /></svg><div style={{position:"absolute",left:n.x+"%",top:n.y+"%",transform:"translate(-50%,-50%)",padding:"8px 16px",borderRadius:8,background:i===p?T+"1A":B3,border:"1.5px solid "+(i<=p?T+"50":BD),fontSize:14,fontWeight:500,color:i<=p?T:DM,transition:"all .5s",whiteSpace:"nowrap",zIndex:1}}>{n.l}{i<=p?" ✓":""}</div></div>})}</div>)}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,minHeight:200}}>
      <div style={{position:"relative",overflow:"hidden"}}>
        <div style={{opacity:ph==="before"?1:.5,transform:ph==="before"?"translateY(0)":"translateY(-8px)",transition:"all .6s cubic-bezier(.22,1,.36,1)",padding:32,borderRadius:14,border:"1.5px solid "+(ph==="before"?"rgba(140,160,200,.15)":BD),background:ph==="before"?B3:BG,height:"100%",display:"flex",flexDirection:"column",justifyContent:"center"}}>
          <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:2,color:PK,marginBottom:12,fontWeight:600}}>Without Sevyn8</div>
  ...
  return (<div>
    ...
    <Sc bt><Rv><Tg>Two paths. One platform.</Tg><h2 style={{...hd,fontSize:34,fontWeight:700,marginBottom:10}}>New deployment or existing infrastructure. Same intelligence.</h2><p style={{fontSize:17,color:MT,marginBottom:40}}>Deploy purpose-built intelligent devices with AI on-board — or add a Sevyn8 box to the sensors you already have. Both paths run the same platform.</p></Rv>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:32}}>
        ...
        <Rv d={0.1}><div style={{padding:32,borderRadius:14,border:"1.5px solid "+AM+"40",background:AM+"06"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><div style={{width:40,height:40,borderRadius:8,background:AM+"18",border:"1.5px solid "+AM+"40",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:AM}}>Hub</div><div style={{fontSize:11,textTransform:"uppercase",letterSpacing:2,color:AM,fontWeight:600}}>Existing infrastructure</div></div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{["Any existing device","One box per site","No rip-and-replace","60-day results"].map(function(t){return <span key={t} style={{fontSize:11,padding:"4px 10px",borderRadius:4,border:"1px solid "+AM+"25",color:AM,background:AM+"08"}}>{t}</span>})}</div>
        </div></Rv>
      </div>
    ...
    <Sc bt><Rv><Tg>Your industry</Tg><h2 style={{...hd,fontSize:34,fontWeight:700,marginBottom:12}}>Find your problem. See the outcome.</h2><p style={{fontSize:17,color:MT,marginBottom:40}}>Click into your industry. Fresh deployment or existing infrastructure — same outcomes.</p></Rv>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        {[{s:SOL[1],pain:"600 cameras, ESL tags, and RFID across 40 stores — and none of them talk to each other. LP reviews 5% of footage after the fact.",out:"40-60% shrinkage reduction. 98%+ footfall accuracy. 60-day POC.",c:T},
          {s:SOL[2],pain:"GPS, dashcams, vibration sensors, facility cameras — four systems, four dashboards, zero correlation.",out:"60% fewer incidents. Continuous compliance. Auditable from day one.",c:AM},
          {s:SOL[3],pain:"Thermal cameras, temperature probes, humidity sensors, door sensors — all logging separately. FSSAI audit prep takes weeks.",out:"Breaches predicted 30 min early. FSSAI automated. Audit prep: hours.",c:V},
          {s:SOL[0],pain:"Cameras, doorbells, motion sensors, smoke detectors — 90 alerts a day from devices that do not talk to each other.",out:"3-5 decisions that matter. Zero data to cloud. Works without WiFi.",c:BL}
...
function SolPg(p){var s=p.s,sc2=useState(0),sci=sc2[0];
  ...
  {s.scn&&<Sc bt><Rv><Tg c={AM}>A day without vs. with</Tg><h2 style={{...hd,fontSize:32,fontWeight:700,marginBottom:28}}>Same moment. Different outcome.</h2></Rv>
    <div style={{display:"flex",gap:6,marginBottom:24}}>{s.scn.map(function(sc,i){return <div key={i} onClick={function(){sc2[1](i)}} style={{padding:"8px 16px",borderRadius:6,cursor:"pointer",background:sci===i?AM+"15":"transparent",border:"1px solid "+(sci===i?AM+"44":BD),fontSize:13,fontWeight:sci===i?600:500,color:sci===i?AM:DM,transition:"all .3s"}}>{sc.t}</div>})}</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      <Rv d={0}><div style={{padding:28,borderRadius:14,border:"1.5px solid "+PK+"30",background:PK+"06",height:"100%"}}>
        <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:2,color:PK,marginBottom:14,fontWeight:600}}>Without Sevyn8</div>
        <div style={{fontSize:13,color:DM,marginBottom:10,fontFamily:"monospace"}}>{s.scn[sci].t}</div>
    ...
    <div style={{display:"flex",justifyContent:"center",gap:4,marginTop:20}}>{s.scn.map(function(sc,i){return <div key={i} style={{width:sci===i?28:8,height:4,borderRadius:2,background:sci===i?AM:BD,transition:"all .4s"}} />})}</div>
  </Sc>}
  ...
  {s.wf&&<Sc bt><Rv><h3 style={{fontSize:11,textTransform:"uppercase",letterSpacing:3,color:PK,marginBottom:12}}>What you are stuck with today</h3><p style={{fontSize:16,color:MT,lineHeight:1.85,maxWidth:620}}>{s.wf}</p></Rv></Sc>}
  ...
  <Sc bt><Rv><Tg>Two deployment paths</Tg><h2 style={{...hd,fontSize:32,fontWeight:700,marginBottom:28}}>New intelligent devices or existing infrastructure.</h2></Rv>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24}}>
      ...
      <Rv d={0.08}><div style={{padding:24,borderRadius:12,border:"1.5px solid "+AM+"35",background:AM+"05"}}>
        <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:2,color:AM,marginBottom:10,fontWeight:600}}>Brownfield</div>
...
function PartPg(p){var cats=[{t:"Silicon Partners (ARM & x86)",i:[["Tier-1 Mobile SoC Provider","Leading ARM-based application processor with dedicated NPU delivering up to 8 TOPS. WiFi 6E integrated. Primary SoC for the Edge AI Box. Design Partnership Agreement signed.","ARM SoC"],["Vision-Grade SoC Provider","4 TOPS dedicated vision processor with 4K ISP. Camera-optimised silicon. STQC ER-5 certified, US-origin. Ideal for surveillance and smart city deployments.","ARM SoC"],["Low-Power IoT SoC Provider","LTE-M/NB-IoT cellular SoC for IoT gateways and BLE 5.4 SoC for ESL and sensor mesh networks. Ultra-low power for battery-operated edge nodes.","IoT SoC"]]},{t:"Security Silicon",i:[["Hardware Security Partner","TPM 2.0 for hardware root of trust. Dedicated secure element for device identity provisioning at scale. Secure MCU for isolated cryptographic operations. DPA signed.","Security"]]},{t:"Sensor & Module Partners",i:[["Global Image Sensor Leader","Industry-leading image sensor family for camera modules. High dynamic range and low-light performance optimised specifically for on-device edge AI inference.","Sensor"],["Pre-Certified Module Provider","Pre-certified LTE Cat1bis, WiFi 6, and secure MCU modules with WPC ETA certification. Saves each OEM partner significant per-SKU compliance cost and time.","Module"],["WiFi 6E Module Specialist","Pre-certified WiFi 6E modules for camera and gateway integration. Regulatory approvals included, accelerating OEM time-to-market.","Module"]]},{t:"Design Partners",i:[["Concept-to-product design","Industrial design, mechanical engineering, thermal management, and design-for-manufacturing for Edge AI Box and Hub hardware.","Design"],["System integration","Full AI-at-Edge stack deployment combining vision, IoT sensors, and edge compute into unified customer solutions.","Integration"]]}];
  return <div style={{paddingTop:80}}><Sc><Rv><Tg>Partners</Tg><h1 style={{...hd,fontSize:44,fontWeight:700,marginBottom:18,maxWidth:500}}>Technology partners. Intelligence infrastructure.</h1><p style={{fontSize:18,color:MT,lineHeight:1.8,maxWidth:540}}>Sevyn8 partners with world-class silicon, sensor, and design firms to build the most capable edge AI platform on the market. We work with the largest ARM and x86 SoC providers, leading global and niche sensor companies, and specialist design houses.</p></Rv></Sc>
    ...
    <Sc bg={B2} bt><Rv><h3 style={{fontSize:11,textTransform:"uppercase",letterSpacing:3,color:DM,marginBottom:22}}>Current customers</h3><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}><Rv d={0}><div style={{padding:24,borderRadius:12,border:"1px solid "+BD,background:BG}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:18,fontWeight:700}}>Global ESL Leader</span><span style={{fontSize:10,color:AM,padding:"3px 10px",border:"1px solid "+AM+"25",borderRadius:4,fontWeight:500}}>Enterprise</span></div><p style={{fontSize:15,color:MT,lineHeight:1.75}}>One of the world's largest Electronic Shelf Label companies. Sevyn8 platform deployed across their AI-at-Edge retail infrastructure combining ESL, RFID, and camera intelligence into a unified solution.</p></div></Rv><Rv d={0.08}><div style={{padding:24,borderRadius:12,border:"1px solid "+BD,background:BG}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:18,fontWeight:700}}>Premium Fashion Retailer</span><span style={{fontSize:10,color:AM,padding:"3px 10px",border:"1px solid "+AM+"25",borderRadius:4,fontWeight:500}}>Enterprise</span></div><p style={{fontSize:15,color:MT,lineHeight:1.75}}>A global-scale premium fashion retail brand. Sevyn8 powers in-store analytics including footfall, conversion tracking, and loss prevention across their retail network.</p></div></Rv></div></Rv></Sc>
...
function AboutPg(){return <div style={{paddingTop:80}}>
  ...
  <Sc bt><Rv><Tg c={AM}>The origin</Tg><h2 style={{...hd,fontSize:34,fontWeight:700,marginBottom:20}}>Forged at Motorola.</h2></Rv>
  ...
  <Sc bg={B2} bt><Rv><Tg c={AM}>The eight</Tg><h2 style={{...hd,fontSize:34,fontWeight:700,marginBottom:20}}>The compounding layer.</h2></Rv>
  ...
  <Sc bt><Rv><Tg c={V}>The team</Tg><h2 style={{...hd,fontSize:34,fontWeight:700,marginBottom:20}}>Built across two decades. Not assembled.</h2></Rv>
    ...
    <Rv d={0.15}><p style={{fontSize:17,color:T,fontWeight:500}}>Skills can be hired. Years of scars and playbooks can only be earned.</p></Rv>
  </Sc>
